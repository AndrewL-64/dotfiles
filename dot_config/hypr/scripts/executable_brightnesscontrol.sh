...
function send_notification {
    ...
    brightness=`brightnessctl -d amdgpu_bl1 | grep -oP "(?<=\()\d+(?=%)" | cat`
    # brightinfo=$(brightnessctl info | awk -F "'" '/Device/ {print $2}')
    brightinfo=$(brightnessctl -d amdgpu_bl1 | awk -F "'" '/Device/ {print $2}')
...
function get_brightness {
    brightnessctl -d amdgpu_bl1 -m | grep -o '[0-9]\+%' | head -c-2
}
